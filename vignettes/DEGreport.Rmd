...
It contains a gene count matrix for 85 TSI HapMap individuals, and the 
gender information. With that, we are going to apply the `glmFit` 
function or `r Biocpkg("DESeq2")` to get genes differentially expressed 
...
Not super useful for many samples because the plot becomes crowded.
...
p-value distribution gives an idea on how well your model is capturing the input data
...
`results` function. `r Biocpkg("DESeq2")` offers multiple ways to ask for
...
To get the original results table, use the parameter like this:
...
The table will always be sorted by padj.
...
And it can return the full table for a list:
...
Volcano plot using the output of `r Biocpkg("DESeq2")`. It mainly needs a data.frame with
two columns (logFC and pVal). Specific genes can be plotted using the option
...
Plot top genes colored by group. Very useful for experiments with nested 
...
Another option for plotting genes in wide format:
...
with markdown code inserted, including figures and table with top de-regulated
genes, GO enrichment analysis and heatmaps and PCA plots. If you set 
\Rcode{path\_results}, different files will be saved there.
...
In this section, we show how to detect patterns of expression. Mainly useful when
